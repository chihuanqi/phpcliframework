...
1. 添加一些组件 比如 pdo, redis, 进程通信一些通用的组件类, 甚至可以支持http,ftp,smtp 等, 可以直接配置使用. 关于http可以使用 主进程listen -> PHP消息队列 -> 子进程抢队列    模式
2. 添加任务可选择, start时 可以选择启动的进程, 不启动该框架其他进程, 停止也可以停止想停止的进程,而不是全部进程, 每次启动改的进程组的 错误日志,重定向, 日志 为一句进程组名称命名的不同文件。
3. 任务的执行可以选择 顺序执行  顺序执行可以达到执行完毕某一个进程以后，执行另一个，某一个进程exit不影响另一个执行（如果exit了另一个不执行了那和写在一个程序没啥区别了） 、 并发执行(已经有了)
